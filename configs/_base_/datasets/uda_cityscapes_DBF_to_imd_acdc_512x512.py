...
acdc_train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='Resize', img_scale=(1024, 540)),  # original 1920x1080
    dict(type='RandomCrop', crop_size=crop_size),
    dict(type='RandomFlip', prob=0.5),
    # dict(type='PhotoMetricDistortion'),  # is applied later in hrda.py
    dict(type='Normalize', **img_norm_cfg),
...
data = dict(
    ...
    train=dict(
        ...
        synethic=dict(
            type='FoggyCityscapesDataset',
            data_root=foggy_cityscapes_root,
            img_dir='leftImg8bit_foggy/train',
            ann_dir='gtFine/train',
            pipeline=cityscapes_train_pipeline),
        intermediate=dict(
            type='ACDCRefDataset',
            data_root=f'{ACDC_data_root}/intermediate',
            img_dir='img',
