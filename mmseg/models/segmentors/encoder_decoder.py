...
class EncoderDecoder(BaseSegmentor):
    ...
    def _decode_head_forward_train(self,
        ...
        loss_decode = self.decode_head.forward_train(x, img_metas,
                                                     gt_semantic_seg,
                                                     self.train_cfg,
                                                     seg_weight, return_logits=False)
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
        ...
        seg_logit /= len(imgs)
        seg_entropy = torch.sum(-seg_logit * torch.log(seg_logit + 1e-7), dim=1)  # has been softmaxed in inference
        seg_pred = seg_logit.argmax(dim=1)
        seg_pred = seg_pred.cpu().numpy()
        # unravel batch dim
        seg_pred = list(seg_pred)
        # print("prob.mean", [(np.mean(probs[a]),np.median(probs[a]), np.max(probs[a])) for a in range(len(probs))])
        return seg_pred, seg_entropy, probs, preds
