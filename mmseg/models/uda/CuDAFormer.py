...
@UDA.register_module()
class CuDAFormer(UDADecorator):
    def __init__(self, **cfg):
        super(CuDAFormer, self).__init__(**cfg)
        self.local_iter = 0
        self.true_label = 1
        self.fake_label = 0
        ...
        self.fi_backbone = build_backbone(deepcopy(cfg['model']['backbone']))
        self.teacher_s2m = build_segmentor(deepcopy(cfg['model']))
        self.teacher_m2t = build_segmentor(deepcopy(cfg['model']))
        # self.dis_r2f = build_discriminator(deepcopy(cfg['discriminator']))
        # self.dis_f2r = build_discriminator(deepcopy(cfg['discriminator']))
    ...
    def forward_train(self,
        ...
        if self.local_iter >= 0:
            ...
            pseudo_weight_imd = torch.sum(ps_large_p_imd).item() / ps_size_imd
            ...
            m2t_mix_masks = get_class_masks(gt_semantic_seg_imd)
            ...
            for i in range(batch_size):
                ...
                m2t_mixed_img[i], m2t_mixed_lbl[i] = strong_transform(
                    ...
                    target=torch.stack((gt_semantic_seg_imd[i][0], pseudo_label_trg[i])))
            ...
            m2t_mix_losses = self.get_model().forward_train(
                m2t_mixed_img, imd_img_metas, m2t_mixed_lbl, pseudo_weight_trg, return_feat=True)
            m2t_mix_losses.pop('features')
        ...
        if self.local_iter % self.debug_img_interval == 0:
            ...
            for j in range(batch_size):
                ...
                subplotimg(axs[1][1], gt_semantic_seg_imd[j], 'Intermediate Seg GT', cmap='cityscapes')
