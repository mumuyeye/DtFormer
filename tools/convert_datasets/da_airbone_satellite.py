...
def convert_to_train_id(file):
    ...
    if label.shape[-1] == 3:
        label = np.bool_(np.sum(label,axis=2))
    id_to_trainid = {
        # False: 0,
        True: 0,
    }
    ...
    new_file = file.replace('.tif', '_labelTrainIds.tif')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert airbone and satellitda_airbone_satellitee seg images annotations to TrainIds')
    parser.add_argument('--path', help='airbone or satellite data path',
                        default='/raid/wzq/data/DA_airbone_satellite/airbone/train/')
...
def main():
    args = parse_args()
    path = args.path
    out_dir = args.out_dir if args.out_dir else path
    mmcv.mkdir_or_exist(out_dir)

    gt_dir = osp.join(path, args.gt_dir)
